...
int main(int argc, char **argv) {
	...
	if (f1.intensity.rbegin()->first > maximal_road_width)
		maximal_road_width = f1.intensity.rbegin()->first;
	if (f2.intensity.rbegin()->first > maximal_road_width)
		maximal_road_width = f2.intensity.rbegin()->first;
	if (f13.intensity.rbegin()->first > maximal_road_width)
		maximal_road_width = f13.intensity.rbegin()->first;
	if (f14.intensity.rbegin()->first > maximal_road_width)
		maximal_road_width = f14.intensity.rbegin()->first;
	...
	if (true) {
		...
		radiation.push_back(
				JRadiationSource(Oxygen, DENSITY_SEA_WATER * JSeaWater::O,
						EErad));
		radiation.push_back(
				JRadiationSource(Chlorine, DENSITY_SEA_WATER * JSeaWater::Cl,
						EErad));
		radiation.push_back(
				JRadiationSource(Hydrogen, DENSITY_SEA_WATER * JSeaWater::H,
						EErad));
	...
	for (vector<string>::const_iterator i = inputFile.begin();
			i != inputFile.end(); ++i)
	...
	NOTICE(
			"Monte Carlo Event tree: " << MonteCarloEvent_Reader.GetEntries() << " " << header.getNumberOfHeaders() << endl);
	...
	if (!header.can) {
		...
		header.can = MONTE_CARLO::can(cyl.getZmin(), cyl.getZmax(),
				cyl.getRadius());
	...
	if (header.coord_origin) {
		...
		detector -= JPoint3D(header.coord_origin.x, header.coord_origin.y,
	...
	for (Long64_t event_count = 0; event_count < numberOfEvents;
			++event_count) {

		STATUS("event: " << setw(10) << event_count << '\r');
		DEBUG(endl);
		...
		// make a modifiable copy of TrackList. Use this copy from now on.
		vector<McTrack> TrackList_copy = event.TrackList();
		vector<McTrack>::iterator track;

		// filter muons from TrackList
		for (track = TrackList_copy.begin(); track != TrackList_copy.end();) {
			// if not (anti) muon, remove from TrackList
			if (!is_muon(*track)) {
				track = TrackList_copy.erase(track);
			} else {
				++track;
			}
		}

		// arrays to hold step, Es values
		double* step_arr = NULL;
		double* Es_arr = NULL;
		const int ARR_SIZE = 200000;

		step_arr = (double*) calloc(ARR_SIZE, sizeof(double));
		Es_arr = (double*) calloc(ARR_SIZE, sizeof(double));
		...
		for (track = TrackList_copy.begin(); track != TrackList_copy.end();
			...
			const pair<double, double> intersection = getIntersection(*track,
					header.can);
			...
			if (Zbed > track->position().z) {
				...
				const double ds = (Zbed - track->position().z)
						/ track->direction().z;
			...
			const JDetectorSubset subdetector(detector, getGeometry(*track),
					maximal_road_width);
			...
			while (E0 > MASS_MUON * getIndexOfRefraction() && temp_z0 < Zmax) {
			...
			for (int k = 0; k < numel; ++k) {
				...
				for (JDetector::const_iterator module = subdetector.lower_bound(
						z0); module != subdetector.end(); ++module) {

					const double R = module->getX();
					const double Z = z0 - module->getZ();

					if (Z + step_arr[k] < -maximal_road_width / getTanThetaC())
						break;

					if (Z <= -R / getTanThetaC()
							&& Z + step_arr[k] > -R / getTanThetaC()) {
						...
						for (int i = 0; i != 2; ++i) {
							...
							if (R < Rmax) {
								...
								try {
									...
									const double NPE = CDF[i].integral->getNPE(
											R) * module->size();
				...
				if (Es_arr[k] >= Ecut) {
					...
					if (writeEMShowers) {
						JPoint3D showerPosition(trackPosition.getX(),
								trackPosition.getY(), z0 + step_arr[k]);
						showerPosition.rotate_back(trackRotation);

						Secondary showerOnTrack;
						showerOnTrack.set_id(event.secondary().size() + 1);
						showerOnTrack.set_t(t0 + step_arr[k] / C);
						showerOnTrack.set_position(showerPosition.getX(),
								showerPosition.getY(), showerPosition.getZ());
						showerOnTrack.set_direction(trackDirection.getDX(),
								trackDirection.getDY(), trackDirection.getDZ());
						showerOnTrack.set_E(Es_arr[k]);
					...
					for (JDetector::const_iterator module =
							subdetector.lower_bound(z0 - maximal_road_width);
							module != subdetector.end(); ++module) {

						const double R = module->getX();
						const double Z = z0 - module->getZ();

						if (Z < -maximal_road_width)
							break;
						...
						for (int i = 0; i != 2; ++i) {
							...
							if (D < Dmax) {
								...
								try {
									...
									const double NPE = CDG[i].integral->getNPE(
											D, cd) * Es_arr[k] * module->size();
									const int N = gRandom->Poisson(NPE);
		...
		if (event.hasNeutrino() && !event.HitList().empty())
			cpu.Fill(log10(event.neutrino().E()),
	...
	NOTICE(
			left << setw(30) << "Number of events input      " << right << setw(10) << (int) job.GetBinContent( 1) << endl);
	NOTICE(
			left << setw(30) << "Number of muons             " << right << setw(10) << (int) job.GetBinContent( 2) << endl);
	NOTICE(
			left << setw(30) << "Number of muons in can      " << right << setw(10) << (int) job.GetBinContent( 3) << endl);
	NOTICE(
			left << setw(30) << "Number of muons within road " << right << setw(10) << (int) job.GetBinContent( 4) << endl);
	NOTICE(
			left << setw(30) << "Number of events output     " << right << setw(10) << (int) job.GetBinContent( 5) << endl);

	NOTICE(
			left << setw(30) << "Number of photons  1        " << right << setw(10) << (int) job.GetBinContent(101) << endl);
	NOTICE(
			left << setw(30) << "Number of photons  2        " << right << setw(10) << (int) job.GetBinContent(102) << endl);
	NOTICE(
			left << setw(30) << "Number of photons 13        " << right << setw(10) << (int) job.GetBinContent(113) << endl);
	NOTICE(
			left << setw(30) << "Number of photons 14        " << right << setw(10) << (int) job.GetBinContent(114) << endl);
	NOTICE(
			left << setw(30) << "Number of photons 23        " << right << setw(10) << (int) job.GetBinContent(123) << endl);
	NOTICE(
			left << setw(30) << "Number of photons 24        " << right << setw(10) << (int) job.GetBinContent(124) << endl);

	NOTICE(
			left << setw(30) << "Number of errors   1        " << right << setw(10) << (int) job.GetBinContent(201) << endl);
	NOTICE(
			left << setw(30) << "Number of errors   2        " << right << setw(10) << (int) job.GetBinContent(202) << endl);
	NOTICE(
			left << setw(30) << "Number of errors  13        " << right << setw(10) << (int) job.GetBinContent(213) << endl);
	NOTICE(
			left << setw(30) << "Number of errors  14        " << right << setw(10) << (int) job.GetBinContent(214) << endl);
	NOTICE(
			left << setw(30) << "Number of errors  23        " << right << setw(10) << (int) job.GetBinContent(223) << endl);
	NOTICE(
			left << setw(30) << "Number of errors  24        " << right << setw(10) << (int) job.GetBinContent(224) << endl);

	printf("\n%d loops", counter);
	printf("\nTotal time = %f sec.", total_time_end - total_time_start);
